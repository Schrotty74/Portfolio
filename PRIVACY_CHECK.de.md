...
- Das Portfolio selbst enthält keine Analysewerkzeuge, Werbung, Formulare, Chatbots, eingebetteten Drittanbieter-Widgets oder eigenen Cookies. Es lädt keine externen Schriften. Die einzige lokale Browser-Speicherung ist die Theme-Präferenz unter `portfolio-theme`; in `localStorage` wird ausschließlich `dark`, `light` oder `color` gespeichert, damit das gewählte Theme wiederhergestellt werden kann.
- Der tägliche Release-Status-Workflow liest öffentliche GitHub-Release-Metadaten und schreibt nur öffentliche Stufe, Version, Zeitstempel und vorhandene DMG-/ZIP-Download-URLs der neuesten Final- und Beta-Version in `release-status.json`.
